...
class Earning extends CI_Model
{
    ...
    public function process_binary($id, $data)
    {
        ...
        $per_user_earning = 3000;
        $sponsor_matching = (($per_user_earning * 20) / 100);

        $sponsor = $this->db_model->select('sponsor', 'member', array('id' => $id));
        $spo = $this->db->select('id')->where('sponsor', $id)->get('member')->result_array();
        //foreach($spo as $sp){
        if ($paid_pair <= 0 && $pair_max >= config_item('binary_frst_ratio') && $pair_match >= config_item('binary_2nd_ratio')) {
            ...
            // if(!empty($sponsor)){
            //     $this->pay_earning($sponsor, $id, 'Sponsor Matching Income', $sponsor_matching, "");
            // }
            if ($data['total_a'] > 0) {
            ...
            for ($i = 0; $i < $pair_match; $i++) {
                ...
                if ($paid_pair == "2") {
                    $this->pay_earning($id, '', 'Matching Income', $per_user_earning, "1");
                    //$this->pay_earning($sponsor, $id, 'Sponsor Matching Income', $sponsor_matching, "");
                    echo  "True" . $id . '' . 'Matching Income' . $per_user_earning . "1 loop-" . $i . "<br>";
                } else {
                    $this->pay_earning($id, '', 'Matching Income', $per_user_earning, "1");
                    //$this->pay_earning($sponsor, $id, 'Sponsor Matching Income', $sponsor_matching, "");
                    echo  "False" . $id . '' . 'Matching Income' . $per_user_earning . "1 loop-" . $i . "<br>";
                }
            }
            ...
            if ($cappping <= 0 || $total_paid < $cappping) {
                ...
                /* if ($min > $avl_cap && $cappping > 0) {
                        $min = $min = $avl_cap;
                        //echo $min.'<br>';
                        $this->pay_earning($id, '', 'Matching Income', $per_user_earning, $pair_match);
                        if(!empty($sponsor)){
                            $this->pay_earning($sponsor, $id, 'Sponsor Matching Income', $sponsor_matching, "");
                        }
                    } */
    ...
    public function reg_earning($userid, $position, $sponsor, $packageid, $need_topup = true, $qty = 1)
    {
        ...
        if ($get_topup > "0" or $need_topup === false) :
            ...
            if (trim($data->level_income) !== "") {
                ...
                foreach ($ex as $e) {
                    ...
                    if ($pay_position > 0 && $e > 0) {
                        $this->pay_earning($pay_position, $userid, 'Level Income', $e);
                    }
                    $i++;
                }
            }
    ...
    {
        // echo "finding  upline $position<br>";
        if ($i > 0) {
            $this->db->select('position')->from('member')->where(array('id' => $position));
            $result = $this->db->get()->row();
            if (!$result) {
                return false;
            } else {
 ...
 */
    ...
    public function auto_earning($table, $userid, $position, $sponsor, $packageid, $need_topup = true, $qty = 1)
    {



        $get_topup = $this->db_model->select('topup', $table, array('member_id' => $userid));
        if ($get_topup > "0" or $need_topup === false) :
            ...
            if ($data->pv > "0") {
                $mypv = $this->db_model->select('mypv', $table, array('member_id' => $userid)) + ($data->pv * $qty);
                $arr  = array('mypv' => $mypv);
            ...
            if (trim($data->level_income) !== "") {
                ...
                foreach ($ex as $e) {
                    ...
                    if ($i == 0) {
                        ...
                        echo "hello2<br/>";                        
                        $pay_position = $this->find_level_position_auto($table,$position, $i);
                    }

                    echo $pay_position."=======".$e."============"."<br/>";

                    if ($pay_position > 0 && $e > 0) {
                        echo "hello3<br/>";                        
                        $this->pay_earning($pay_position, $userid, 'Auto Level Income', $e);
                        echo "HELLO2" . $this->db->last_query();
                        // die;
                    }
                    $i++;
                }
    ...
    private function find_level_position_auto($table,$position, $i)
    {
        // echo "finding  upline $position<br>";
        if ($i > 0) {
            $this->db->select('position')->from($table)->where(array('id' => $position));
            $result = $this->db->get()->row();
            if (!$result) {
                return false;
            } else {
                ...
                return $this->find_level_position_auto($table,$result->position, $i);
            }
        } else {
            return $position;
        }
